...
| `gate-v1-indexes` | `req-v1-secondary-index-proof` | none | No current secondary-index implementation or range-scan evidence exists in this task; this remains explicit missing evidence rather than completed progress. | `blocked_missing_evidence` |
...
V1 remains a single-process Rust CLI database. This guide does not claim network service behavior, multi-process concurrency, distributed storage, public benchmark CLI support, secondary-index completion, or performance beyond the lower-bound workload documented in `docs/benchmarks.md`.
